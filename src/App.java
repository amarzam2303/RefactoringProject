...
public class App {
    ...
    private static void generarFicheroPedido(Pedido pedido){
        try {
            ...
            myWriter.write(String.format("Cliente: %s", pedido.getCliente().getNombre()));
            myWriter.write(String.format("Direccion: %s", pedido.getCliente().getDireccion()));
            myWriter.write(String.format("Total a pagar: %.2f", pedido.aplicarSubtotalConIVA()));
            myWriter.close();
    ...
    private static void imprimirPedido(Pedido pedido){
        ...
        System.out.println(String.format("Total con IVA (%.2f %)" , (IVA*100) , pedido.getCliente().getId()));
        System.out.println("--------------------------------------------------");
    }
}
